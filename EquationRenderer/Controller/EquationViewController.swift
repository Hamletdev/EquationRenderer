...
class EquationViewController: UIViewController, UITextFieldDelegate {
    ...
    //MARK: - NSCODER SAVE AND READ
    func saveRenderedImages() {
        let encoder = PropertyListEncoder()
        do {
            let encodedData = try encoder.encode(imageCache)
    ...
    func loadRenderedImages() {
        if let data = try? Data(contentsOf: dataFilePath!) {
            let decodedData = PropertyListDecoder()
            do {
                imageCache = try decodedData.decode([String: Data].self, from: data)
                ...
                print("Error in Decoding plist file")
            }
